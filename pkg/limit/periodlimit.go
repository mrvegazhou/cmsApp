...
type (
	// 如果参数非常多，推荐使用此模式来设置参数 go中常见的option参数模式
...
func NewPeriodLimit(period, quota int, keyPrefix string, opts ...PeriodOption) *PeriodLimit {
	limiter := &PeriodLimit{
		period:     period,
		quota:      quota,
		limitStore: periodScript,
		keyPrefix:  keyPrefix,
	}
...
func (h *PeriodLimit) TakeCtx(ctx context.Context, key string) (int, error) {
	...
	code, err := h.limitStore.Run(ctx, rdb, []string{h.keyPrefix + key}, []string{
