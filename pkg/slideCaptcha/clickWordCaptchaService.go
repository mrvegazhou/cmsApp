...
func (c *ClickWordCaptchaService) randomWordPoint(width int, height int, i int, count int) SlideCaptchaPoint {
	avgWidth := width / (count + 1)
	fontSizeHalf := c.factory.config.ClickWord.FontSize / 2

	var x, y int
	if avgWidth < fontSizeHalf {
		x = random.RandomInt(1+fontSizeHalf, width)
	} else {
		if i == 0 {
			x = random.RandomInt(1+fontSizeHalf, avgWidth*(i+1)-fontSizeHalf)
		} else {
			x = random.RandomInt(avgWidth*i+fontSizeHalf, avgWidth*(i+1)-fontSizeHalf)
		}
	}
	y = random.RandomInt(c.factory.config.ClickWord.FontSize, height-fontSizeHalf)
	return SlideCaptchaPoint{X: x, Y: y}
}

// getRandomWords 获取随机文字
func (c *ClickWordCaptchaService) getRandomWords(count int) []string {
	...
	for {
		word := runesArray[random.RandomInt(0, size-1)]
...
func (c *ClickWordCaptchaService) getImageData(image *ImageUtil) ([]SlideCaptchaPoint, []string, error) {
	wordCount := c.factory.config.ClickWord.FontNum

	// 某个字不参与校验
	num := random.RandomInt(1, wordCount)
	currentWord := c.getRandomWords(wordCount)

	var pointList []SlideCaptchaPoint
	var wordList []string

	i := 0

	// 构建本次的 secret
	key := random.RandString(16)
...
func (c *ClickWordCaptchaService) Get() (map[string]interface{}, error) {
	// 初始化背景图片
	backgroundImage := GetClickBackgroundImage()
...
func (c *ClickWordCaptchaService) Check(token string, pointJson string) error {
	...
	// 解析结构体
	var cachePoint []SlideCaptchaPoint

	var userPoint []SlideCaptchaPoint
